...
const convertLichessGame = async (
  ...
  if (!pgn || pgn.trim() === "") {
    try {
      ...
      console.warn(`Impossible de récupérer le PGN pour la partie ${apiGame.id}:`, error);
  ...
  else if (apiGame.status === "mate" && apiGame.winner === "black") result = "0-1";
  else if (apiGame.status === "draw" || apiGame.status === "stalemate") result = "1/2-1/2";
...
export const prepareGamesForInsert = async (
  ...
  userId: string,
): Promise<Array<Omit<Game, "id" | "imported_at" | "analyzed_at">>> => {
  const games: Array<Omit<Game, "id" | "imported_at" | "analyzed_at">> = [];
  ...
  for (let i = 0; i < apiGames.length; i++) {
    ...
    try {
      if (platform === "chesscom") {
        ...
        const converted = await convertLichessGame(apiGame as LichessGame, userId);
        games.push(converted);
      }
    } catch (error: any) {
      console.error(
  ...
  console.log(`[Sync] ${games.length}/${apiGames.length} parties converties avec succès`);
